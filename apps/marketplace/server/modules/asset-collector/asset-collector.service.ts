...
export default class AssetCollectorService {
	private getFileProps(file: string, isEntry = false): Record<string, unknown> | null {
		...
		else if (file.endsWith('.gif')) return { rel: 'preload', href: file, as: 'image', type: 'image/gif' }
		else if (file.endsWith('.jpg') || file.endsWith('.jpeg'))
			return { rel: 'preload', href: file, as: 'image', type: 'image/jpeg' }
		else if (file.endsWith('.png')) return { rel: 'preload', href: file, as: 'image', type: 'image/png' }
		else return null
	}
