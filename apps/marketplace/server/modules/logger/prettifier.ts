...
const joinMsg = (...arr: Array<string | undefined>): string => arr.filter((item): item is string => !!item).join(' ')
...
export const messageFormat: PinoPretty.MessageFormatFunc = (log) => {
	...
	if (res) {
		...
		return joinMsg(prettyTransport, colorFn('-->'), res.method, colorFn(res.statusCode), res.url, ms)
	}
